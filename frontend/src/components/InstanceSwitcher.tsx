...
export default function InstanceSwitcher() {
  ...
  const formatStats = (inst: InstanceInfo): string => {
    ...
    if (inst.mcp_server_count > 0) {
      parts.push(`${inst.mcp_server_count} MCP`);
    }
    if (inst.settings_count > 0) {
      parts.push(`${inst.settings_count} settings`);
    }
    if (parts.length === 0) return "No config";
    return parts.join(", ");
  };
  ...
  return (
    <>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, flex: 1, minWidth: 0 }}>
                ...
                <Box sx={{ minWidth: 0 }}>
                  <Typography
                    variant="body2"
                    sx={{
                      fontWeight: inst.is_active ? 600 : 400,
                      fontSize: "0.8rem",
                      overflow: "hidden",
                      textOverflow: "ellipsis",
                      whiteSpace: "nowrap",
              ...
              {!inst.is_active && (
                <Tooltip title="Remove instance">
                  <IconButton
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setDeleteConfirmPath(inst.path);
                    }}
                    sx={{
                      p: 0.25,
                      opacity: 0.5,
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setDeleteConfirmPath(null)} color="inherit">
            Cancel
          </Button>
          <Button
