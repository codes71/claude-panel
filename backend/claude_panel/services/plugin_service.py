...
from claude_panel.services.settings_service import get_enabled_plugins, set_plugin_enabled
from claude_panel.services.token_estimator import PLUGIN_BASE_TOKENS, PLUGIN_SKILL_TOKENS, PLUGIN_AGENT_TOKENS
...
def _resolve_plugin_root(config: dict, install_path: str) -> dict:
    """Replace ${CLAUDE_PLUGIN_ROOT} in command/args/env values."""
    resolved = {}
    for key, value in config.items():
        if isinstance(value, str):
            resolved[key] = value.replace("${CLAUDE_PLUGIN_ROOT}", install_path)
        elif isinstance(value, list):
            resolved[key] = [
                v.replace("${CLAUDE_PLUGIN_ROOT}", install_path) if isinstance(v, str) else v
                for v in value
            ]
        elif isinstance(value, dict):
            resolved[key] = {
                k: v.replace("${CLAUDE_PLUGIN_ROOT}", install_path) if isinstance(v, str) else v
...
def list_plugin_mcp_entries() -> list[dict]:
    """Return MCP server entries from enabled plugins' .mcp.json files."""
    installed = _read_installed()
    enabled = get_enabled_plugins()
    entries = []
    ...
    for pid, is_enabled in enabled.items():
        ...
        # Get install path from installed_plugins.json
        install_entries = installed.get(pid, [])
        if not install_entries:
            continue
        install_path = install_entries[0].get("installPath", "")
        if not install_path:
            continue
        ...
        if not isinstance(servers, dict):
            continue
        # Skip if we got the wrapper — don't treat "mcpServers" as a server name
